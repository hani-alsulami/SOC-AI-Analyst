"""
MITRE D3FEND Integration - AI-SOC Responder (Cortex)

Maps ATT&CK offensive techniques to D3FEND defensive countermeasures,
then maps those to concrete proposed actions. This is the bridge
between "what attack was detected" and "what defense should be
proposed to the analyst."
...
Ported from services/response-orchestrator/d3fend.py in the main
AI-SOC project — identical mapping data, with the pydantic-based
ActionType/AdapterType/BlastRadius enums inlined here (as plain
str Enums) so this module has no dependency on that FastAPI service.
...
class BlastRadius(str, Enum):
    """Impact scope of a proposed action."""
...
class ActionType(str, Enum):
    """Concrete defense actions the D3FEND mapping can propose."""
    BLOCK_IP = "block_ip"
...
class AdapterType(str, Enum):
    """Which system would execute the action in a fully wired deployment.
    None of these are connected in this Cortex integration yet — see
    README.md. Kept from the original mapping as descriptive metadata."""
    WAZUH = "wazuh"
    FIREWALL = "firewall"
    EDR = "edr"
    IDENTITY = "identity"
